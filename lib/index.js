...
class FileLocationConverter {
  ...
  getKey(file) {
    const hash = crypto.createHash('md5').update(file.hash).digest("hex").slice(0, 12)
    let name = file.name;
    ...
    name = `${name}_${hash}${file.ext}`;
    if (!this.config.directory) return name;
  ...
  getUrl(data) {
    ...
    var parts = {};
    URI.parse(this.config.cdn, parts);
